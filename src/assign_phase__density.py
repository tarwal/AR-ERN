...
#
# Method based on density. For details, cf PhDs. Rev. Materials 5, 126001 (2021)
#
print_log(" ")
print_log("Counting atoms per cell...")
print_log(" ")
#
...
print_log(" ")
print_log("Assigning phase based on the cell density...")
print_log(" ")
#
...
for nx in range(numofcellsalongx):
    for ny in range(numofcellsalongy):
        for nz in range(numofcellsalongz):
            if atomsincell[nx,ny,nz]> (phase_assignment_threshold*(pacex*pacey*pacez)):
                cellphase[nx][ny][nz]='c'
            elif atomsincell[nx][ny][nz]==0:
